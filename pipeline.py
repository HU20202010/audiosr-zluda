...
def seed_everything(seed):
    ...
    # region debug-point seed-cudnn-state
    import os
    ...
    if _is_z:
        ...
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = False
    print(f"[DBG-seed] ZLUDA={os.environ.get('ZLUDA')} cudnn.enabled={torch.backends.cudnn.enabled} benchmark={torch.backends.cudnn.benchmark} deterministic={torch.backends.cudnn.deterministic}", flush=True)
    # endregion
...
def build_model(ckpt_path=None, config=None, device=None, model_name="basic"):
    ...
    # region debug-point build-model-gpu
    import os
    if torch.cuda.is_available():
        print(f"[DBG-build] CUDA device name: {torch.cuda.get_device_name(0)}", flush=True)
    _is_z = bool(os.environ.get("ZLUDA")) or (torch.cuda.is_available() and "NVIDIA" not in torch.cuda.get_device_name(0).upper())
    print(f"[DBG-build] detected ZLUDA/AMD={_is_z}", flush=True)
    # endregion
...
def super_resolution(
    ...
    with torch.no_grad():
        # region debug-point sr-pre-gen
        import torch.backends.cudnn as _cudnn, os
        _is_z = bool(os.environ.get("ZLUDA")) or (torch.cuda.is_available() and "NVIDIA" not in torch.cuda.get_device_name(0).upper())
        if _is_z:
            _cudnn.enabled = False
            _cudnn.benchmark = False
        print(f"[DBG-sr] pre-generate_batch cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark}", flush=True)
        # endregion
