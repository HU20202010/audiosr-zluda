...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if return_first_stage_encode:
            # region debug-point ddpm-pre-encode
            import torch.backends.cudnn as _cudnn, os
            if os.environ.get("ZLUDA"):
                _cudnn.enabled = False
                _cudnn.benchmark = False
            print(f"[DBG-ddpm] pre-encode cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark} deterministic={_cudnn.deterministic} device={x.device}", flush=True)
            # endregion
            encoder_posterior = self.encode_first_stage(x)
            z = self.get_first_stage_encoding(encoder_posterior).detach()
        else:
    ...
    def decode_first_stage(self, z):
        with torch.no_grad():
            # region debug-point decode-cudnn-state
            import torch.backends.cudnn as _cudnn, os
            _is_z = bool(os.environ.get("ZLUDA")) or (torch.cuda.is_available() and "NVIDIA" not in torch.cuda.get_device_name(0).upper())
            if _is_z:
                _cudnn.enabled = False
                _cudnn.benchmark = False
            print(f"[DBG-decode] cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark}", flush=True)
            # endregion
    ...
    def mel_spectrogram_to_waveform(
        ...
        # region debug-point vocoder-cudnn-state
        import torch.backends.cudnn as _cudnn, os
        _is_z = bool(os.environ.get("ZLUDA")) or (torch.cuda.is_available() and "NVIDIA" not in torch.cuda.get_device_name(0).upper())
        if _is_z:
            _cudnn.enabled = False
            _cudnn.benchmark = False
        print(f"[DBG-vocoder] cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark}", flush=True)
        # endregion
    ...
    def encode_first_stage(self, x):
        with torch.no_grad():
            # region debug-point encode-cudnn-state
            import torch.backends.cudnn as _cudnn, os
            _is_z = bool(os.environ.get("ZLUDA")) or (torch.cuda.is_available() and "NVIDIA" not in torch.cuda.get_device_name(0).upper())
            if _is_z:
                _cudnn.enabled = False
                _cudnn.benchmark = False
            print(f"[DBG-encode-fs] cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark}", flush=True)
            # endregion
