...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        temb = None
        # downsampling
        # region debug-point conv-cudnn-state
        import torch.backends.cudnn as _cudnn, os
        if os.environ.get("ZLUDA"):
            _cudnn.enabled = False
            _cudnn.benchmark = False
        print(f"[DBG-conv] cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark} deterministic={_cudnn.deterministic} input_device={x.device}", flush=True)
        # endregion
        hs = [self.conv_in(x)]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
