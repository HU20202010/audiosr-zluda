...
class AutoencoderKL(nn.Module):
    ...
    def encode(self, x):
        ...
        # region debug-point encoder-cudnn-state
        import torch.backends.cudnn as _cudnn, os
        if os.environ.get("ZLUDA"):
            _cudnn.enabled = False
            _cudnn.benchmark = False
        print(f"[DBG-encoder] cudnn.enabled={_cudnn.enabled} benchmark={_cudnn.benchmark} deterministic={_cudnn.deterministic} input_device={x.device}", flush=True)
        # endregion
        h = self.encoder(x)
